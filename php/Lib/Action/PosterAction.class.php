...
class PosterAction extends Action {
	public function index() {
		$m = D('Poster');
		$this->assign('stat', $this->get_stat($m));
		$this->display();
	}
	...
	private function parseInput() {
		...
		if (!empty($_GET['order'])) {
			...
			case 'follow': $cond[] = "EXISTS (SELECT * FROM ustc_follow_org AS f WHERE f.uid = '".getUid()."' AND gid = f.gid)"; break;
			}
		}
	...
	public function ajaxGet() {
		list($start, $num, $cond, $order) = parseInput();
