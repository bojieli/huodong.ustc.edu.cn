<?php
// include/class_user.php 2011-07-10 boj
...
class user {
	...
	function init() {
		...
		if(!empty($_COOKIE['auth'])) {
			$auth = daddslashes(explode("\t", authcode($_COOKIE['auth'], 'DECODE')));
		}
		if (empty($auth) || count($auth) != 2) {
			$password = $uid = null;
			define('CURRENT_USER', 0); // user 0 is guest
		} else {
			$password = $auth[0];
			$uid = $auth[1];
			define('CURRENT_USER', $uid);
		}*/
	...
	function login($loginname, $password) {
		...
		if (user::login_too_many_times()) {
			return LOGIN_TOO_MANY_TIMES;
		}
		if (empty($user)) {
			user::record_false_login($loginname, $password);			
			return LOGIN_NAME_NOT_EXIST;		
		}
		if (md5(md5($password).$user['salt']) !== $user['password']) {
			user::record_false_login($loginname, md5($password));			
