...
<li class="pre"><a href="js_loop_for_in.asp">Previous Page</a></li>

<li class="next"><a href="js_try_catch.asp">Next Page</a></li>
</ul>

</div>

<div id="intro">
<p><strong>事件是可以被 JavaScript 侦测到的行为。</strong></p>
</div>
...
<li>鼠标点击</li>
<li>页面或图像载入</li>
<li>鼠标悬浮于页面的某个热点之上</li>
<li>在表单中选取输入框</li>
<li>确认表单</li>
<li>键盘按键</li>
</ul>
<p class="important"><span>注意：</span>事件通常与函数配合使用，当事件发生时函数才会执行。</p>
<p>如果需要更多有关 Javascript 可识别事件的知识，请阅读我们的 <a href="jsref_events.asp">JavaScript 事件参考手册</a>。</p>
</div>

<div>
<h2>onload 和 onUnload</h2>
<p>当用户进入或离开页面时就会触发 onload 和 onUnload 事件。</p>
<p>onload 事件常用来检测访问者的浏览器类型和版本，然后根据这些信息载入特定版本的网页。</p>
<p>onload 和 onUnload 事件也常被用来处理用户进入或离开页面时所建立的 cookies。例如，当某用户第一次进入页面时，你可以使用消息框来询问用户的姓名。姓名会保存在 cookie 中。当用户再次进入这个页面时，你可以使用另一个消息框来和这个用户打招呼：&quot;Welcome John Doe!&quot;。</p>
</div>

<div>
<h2>onFocus, onBlur 和 onChange</h2>
<p>onFocus、onBlur 和 onChange 事件通常相互配合用来验证表单。</p>
<p>下面是一个使用 onChange 事件的例子。用户一旦改变了域的内容，checkEmail() 函数就会被调用。</p>
<pre>&lt;input type=&quot;text&quot; size=&quot;30&quot; id=&quot;email&quot; <code>onchange=&quot;checkEmail()</code>&quot;&gt;</pre>
</div>

<div>
<h2>onSubmit</h2>
<p>onSubmit 用于在提交表单之前验证所有的表单域。</p>
<p>下面是一个使用 onSubmit 事件的例子。当用户单击表单中的确认按钮时，checkForm() 函数就会被调用。假若域的值无效，此次提交就会被取消。checkForm() 函数的返回值是 true 或者 false。如果返回值为true，则提交表单，反之取消提交。</p>
<pre>&lt;form method=&quot;post&quot; action=&quot;xxx.htm&quot; <code>onsubmit=&quot;return checkForm()&quot;&gt;</code></pre>
</div>

<div>
<h2>onMouseOver 和 onMouseOut</h2>
<p>onMouseOver 和 onMouseOut 用来创建“动态的”按钮。</p>
<p>下面是一个使用 onMouseOver 事件的例子。当 onMouseOver 事件被脚本侦测到时，就会弹出一个警告框：</p>
<pre>&lt;a href=&quot;http://www.w3school.com.cn&quot;
<code>onmouseover=&quot;alert('An onMouseOver event');return false&quot;</code>&gt;
