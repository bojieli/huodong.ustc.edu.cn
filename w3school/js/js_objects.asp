...
<title>创建你自己的 JavaScript 对象</title>
</head>
...
<h1>创建你自己的 JavaScript 对象</h1>

<div id="tpn">
<ul class="prenext">

<li class="pre"><a href="js_timing.asp">Previous Page</a></li>

<li class="next"><a href="js_summary.asp">Next Page</a></li>
</ul>
...
<dd>本例向您展示如何创建 JavaScript 对象的实例 (instance)。</dd>

<dt><a href="../tiy/t.asp@f=jseg_create_object2">创建用于对象的模板</a></dt>
<dd>本例向您展示如何创建 JavaScript 对象的模板 (template)。</dd>
</dl>
</div>
...
<p>在本教程前面的章节，我们已经学习到 JavaScript 拥有若干内置的对象，比如 String、Date、Array 等等。除了这些对象，你还可以创建自己的对象。</p>
<p>对象仅仅是一种特殊的数据类型而已，并拥有一系列的属性和方法。</p>
<p>让我们用一个例子来理解：一个人就是一个对象。属性是和对象有关的值。人的属性包括其名字、身高、体重、年纪、肤色、眼睛的颜色等等。所有的人都有这些属性，但是每个人的属性的值却各不相同。对象也拥有方法。方法是可施加于对象上的行为。人的方法可能是吃、睡、工作、玩等等。</p>
<h3>属性</h3>
<p>访问对象的属性的方法：</p>
<pre>对象名.属性名</pre>
<p>通过简单地向属性赋值，你就可以向对象添加属性。假定存在 personObj 这个对象 － 你可以添加诸如 firstname、lastname、age 以及 eyecolor 等属性。</p>
<pre>personObj.firstname=&quot;John&quot;
personObj.lastname=&quot;Doe&quot;
personObj.age=30
personObj.eyecolor=&quot;blue&quot;
document.write(personObj.firstname)</pre>
<p>上面的代码生成以下的输出：</p>
<pre>John</pre>
<h3>方法</h3>
<p>对象可包含方法。</p>
<p>使用下面的语法来调用方法:</p>
<pre>对象名.方法名()</pre>
<p class="important"><span>注意：</span>位于括号之间的用于方法的参数是可以省略的。</p>
<p>调用名为 sleep 的 personObj 对象的方法：</p>
...
<h3>1. 创建对象的实例</h3>
<p>下列代码创建了一个对象的实例，并向其添加了四个属性：</p>
<pre>personObj=new Object()
personObj.firstname=&quot;John&quot;
personObj.lastname=&quot;Doe&quot;
personObj.age=50
personObj.eyecolor=&quot;blue&quot;</pre>
<p>向 personObj 添加方法也很简单。下列代码向 personObj 添加了名为 eat() 的方法：</p>
...
<p class="important"><span>注意：</span>模版仅仅是一个函数。你需要在函数内部向 this.propertiName 分配内容。</p>
<p>一旦拥有模版，你就可以创建新的实例，就像这样：</p>
<pre>myFather=new person(&quot;John&quot;,&quot;Doe&quot;,50,&quot;blue&quot;)
myMother=new person(&quot;Sally&quot;,&quot;Rally&quot;,48,&quot;green&quot;)
</pre>
<p>同样可以向 person 对象添加某些方法。并且同样需要在模版内进行操作：</p>
<pre>
...
<p class="important"><span>注意：</span>方法只是依附于对象的函数而已。然后，我们需要编写 newlastname() 函数：</p>
<pre>function newlastname(new_lastname)
{
this.lastname=new_lastname
}</pre>
<p>Newlastname() 函数定义 person 的新的 lastname，并将之分配给 person。通过使用 “this.”，JavaScript 即可得知你指的 person 是谁。因此，现在你可以这样写：myMother.newlastname(&quot;Doe&quot;)。</p>
</div>
