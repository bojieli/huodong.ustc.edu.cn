...
<h1>JavaScript 浏览器检测</h1>

<div id="tpn">
<ul class="prenext">

<li class="pre"><a href="js_obj_htmldom.asp">Previous Page</a></li>

<li class="next"><a href="js_cookies.asp">Next Page</a></li>
</ul>
...
<dd>使用 JavaScript 检测关于访问者的浏览器名称及其版本。</dd>

<dt><a href="../tiy/t.asp@f=jseg_browserdetails">检测浏览器的更多信息</a></dt>
<dd>使用 JavaScript 检测关于访问者浏览器的更多信息。</dd>

<dt><a href="../tiy/t.asp@f=hdom_navigator">检测浏览器的全部信息</a></dt>
<dd>使用 JavaScript 检测关于访问者浏览器的全部信息。</dd>

<dt><a href="../tiy/t.asp@f=jseg_browser2">根据浏览器类型提醒用户</a></dt>
<dd>使用 JavaScript 检测关于访问者的浏览器名称及其版本，然后根据这些信息生成不同内容的警告框。</dd>
</dl>
</div>

<div>
<h2>浏览器检测</h2>
<p>本教程中几乎所有的代码均可在任何支持 JavaScript 的浏览器中运行。不过个别的代码无法运行于特定的浏览器，特别是老式的浏览器。</p>
<p>所以，有些时候对访问者的浏览器类型及版本进行检测是很有帮助的，然后可在此基础上为访问者提供合适的信息。</p>
<p>要做到这一点，最好的办法是使你的网页变得足够聪明，这样的话它就可以不同的方式对待不同类型的浏览器。</p>
<p>JavaScript 包含一个名为 Navigator 的对象，它就可以完成上述的任务。</p>
<p>Navigator 包含了有关访问者浏览器的信息，包括浏览器类型、版本等等。</p>
</div>

<div>
<h2>Navigator 对象</h2>
<p>JavaScript Navigator 对象包含了有关访问者浏览器的所有信息。接下来我们学习 Navigator 对象的两个属性。</p>
<dl class="define">
<dt>appName</dt>
<dd>保存浏览器类型</dd>
<dt>appVersion</dt>
<dd>存有浏览器的版本信息（其他信息中的一项）</dd>
</dl>
<h3>实例</h3>
<pre>&lt;html&gt;
...
&lt;body&gt;
&lt;script type=&quot;text/javascript&quot;&gt;
var browser=<code>navigator.appName</code>
var b_version=<code>navigator.appVersion</code>
...
&lt;/html&gt;</pre>
<p>上面例子中的 browser 变量存有浏览器的名称，比如，&quot;Netscape&quot; 或者 &quot;Microsoft Internet Explorer&quot;。</p>
<p>上面例子中的 appVersion 属性返回的字符串所包含的信息不止是版本号而已，但是现在我们只关注版本号。我们使用一个名为 parseFloat() 的函数会抽取字符串中类似十进制数的一段字符并将之返回，这样我们就可以从字符串中抽出版本号信息了。</p>
<p class="important"><span>重要事项</span>：在 IE 5.0 及以后版本中，版本号是不正确的！在 IE 5.0 和 IE 6.0 中，微软为 appVersion 字符串赋的值是 4.0。怎么会出现这样的错误呢？无论如何，我们需要清楚的是，JavaScript 在 IE6、IE5 和 IE4 中的获得的版本号是相同的。</p>
<h3>实例</h3>
<p>下面的脚本会根据访问者的浏览器类型显示不同的警告。</p>
<pre>&lt;html&gt;
...
{
<code>var browser=navigator.appName
...
&amp;&amp; (version&gt;=4))
  ...
  {alert(&quot;It's time to upgrade your browser!&quot;)}</code>
  }
&lt;/script&gt;
