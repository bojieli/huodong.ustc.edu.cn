...
<li class="pre"><a href="js_functions.asp">Previous Page</a></li>

<li class="next"><a href="js_loop_while.asp">Next Page</a></li>
</ul>
...
<dd>如何编写 loop 循环来按照指定的次数执行相同的代码。</dd>

<dt><a href="../tiy/t.asp@f=jseg_fornext_header">循环产生 HTML 标题</a></dt>
<dd>如何使用Loop循环来产生不同的HTML标题。</dd>
</dl>
</div>
...
<p>在编写代码时，你常常希望反复执行同一段代码。我们可以使用循环来完成这个功能，这样就用不着重复地写若干行相同的代码。</p>

<h3>JavaScript 有两种不同种类的循环：</h3>
<dl>
<dt>for</dt>
<dd>将一段代码循环执行指定的次数</dd>
<dt>while</dt>
<dd>当指定的条件为 true 时循环执行代码</dd>
</dl>
</div>
...
<p>在脚本的运行次数已确定的情况下使用 for 循环。</p>

<h3>语法：</h3>
<pre><code>for</code> (变量=开始值;变量&lt;=结束值;变量=变量+步进值) 
{
    需执行的代码
}
</pre>
...
<h3>实例：</h3>
<p>解释：下面的例子定义了一个循环程序，这个程序中 i 的起始值为 0。每执行一次循环，i 的值就会累加一次 1，循环会一直运行下去，直到 i 等于 10 为止。</p>
<p class="note"><span>注释：</span>步进值可以为负。如果步进值为负，需要调整 for 声明中的比较运算符。</p>
<pre>&lt;html&gt;
&lt;body&gt;
