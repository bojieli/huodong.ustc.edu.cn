...
<li class="pre"><a href="js_loop_for.asp">Previous Page</a></li>

<li class="next"><a href="js_break.asp">Next Page</a></li>
</ul>

</div>

<div id="intro">
<p><strong>JavaScript 中的循环用来将同一段代码执行指定的次数（或者当指定的条件为 true 时）。</strong></p>
</div>
...
<dd>利用 while 循环在指定条件为 true 时来循环执行代码。</dd>

<dt><a href="../tiy/t.asp@f=jseg_dowhile">Do while 循环</a></dt>
<dd>利用 do...while 循环在指定条件为 true 时来循环执行代码。在即使条件为 false 时，这种循环也会至少执行一次。这是因为在条件被验证前，这个语句就会执行。</dd>
</dl>
</div>

<div>
<h2>while 循环</h2>
<p>while 循环用于在指定条件为 true 时循环执行代码。</p>
<h3>语法：</h3>
<pre><code>while</code> (变量&lt;=结束值)
{
    需执行的代码
}
</pre>
<p class="important"><span>注意：</span>除了&lt;=，还可以使用其他的比较运算符。</p>
<h3>实例：</h3>
<p>解释：下面的例子定义了一个循环程序，这个循环程序的参数 i 的起始值为 0。该程序会反复运行，直到 i 大于 10 为止。i 的步进值为 1。</p>
<pre>&lt;html&gt;
&lt;body&gt;
&lt;script type=&quot;text/javascript&quot;&gt;
var i=0
...
<div>
<h2>do...while 循环</h2>
<p>do...while 循环是 while 循环的变种。该循环程序在初次运行时会首先执行一遍其中的代码，然后当指定的条件为 true 时，它会继续这个循环。所以可以这么说，do...while 循环为执行至少一遍其中的代码，即使条件为 false，因为其中的代码执行后才会进行条件验证。</p>
<h3>语法：</h3>
<pre><code>do</code>
{
    需执行的代码
}
<code>while</code> (变量&lt;=结束值)</pre>
<h3>实例：</h3>
<pre>&lt;html&gt;
&lt;body&gt;
&lt;script type=&quot;text/javascript&quot;&gt;
