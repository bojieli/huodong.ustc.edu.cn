...
<p>文档树中具有指定标记的 Element 节点的只读数组（从技术上讲，是 <a href="dom_nodelist.asp" title="XML DOM - NodeList 对象">NodeList 对象</a>）。返回的元素节点的顺序就是它们在源文档中出现的顺序。</p>


<h3>说明</h3>

<p>该方法将返回一个 <a href="dom_nodelist.asp" title="XML DOM - NodeList 对象">NodeList 对象</a>（可以作为只读数组处理），该对象存放文档中具有指定标签名的所有 Element 节点，它们存放的顺序就是在源文档中出现的顺序。<a href="dom_nodelist.asp" title="XML DOM - NodeList 对象">NodeList 对象</a>是“活”的，即如果在文档中添加或删除了指定标签名的元素，它的内容会自动进行必要的更新。</p>

<p>注意，Element 接口定义了一个同名的方法，该方法只检索文档的子树。另外，HTMLDocument 接口定义了 <a href="../htmldom/met_doc_getelementsbyname.asp" title="HTML DOM getElementsByName() 方法">getElementsByName() 方法</a>，基于 name 属性的值（而不是标签名）检索元素。</p>
