...
<h3>抛出</h3>
<p>如果 <i>newNode</i> 是 Attr、Document、Entity 或 Notation 节点，该方法将抛出代码为 INVALID_NODE_TYPE_ERR 的 <a href="dom_rangeexception.asp" title="XML DOM - RangeException 对象">RangeException 异常</a>。</p>
...
<h3>描述</h3>
<p>该方法将把指定的节点（和它的所有子孙节点）插入文档范围的开始点。当该方法返回时，当前范围将包括新插入的节点。如果 <i>newNode</i> 已经是文档的一部分，那么它将被从当前位置删除，然后重新插入范围的开始点。如果 <i>newNode</i> 是 <a href="dom_documentfragment.asp" title="XML DOM - DocumentFragment 对象">DocumentFragment 节点</a>，那么插入的不是它自身，而是它的子孙节点，按顺序插入范围的开始点。</p>
<p>如果包含当前范围的开始点的节点是 Text 节点，那么在发生插入操作前，它将被分割成两个相邻的节点。如果 <i>newNode</i> 是 Text 节点，在插入文档后，它不会与任何相邻的 Text 节点合并。要合并相邻的节点，需要调用 <a href="met_node_normalize.asp" title="XML DOM normalize() 方法">Node.normalize()</a> 方法。</p>

<h3>参阅</h3>
<p>
<a href="dom_documentfragment.asp" title="XML DOM - DocumentFragment 对象">DocumentFragment 对象</a>、
<a href="met_node_normalize.asp" title="XML DOM normalize() 方法">Node.normalize()</a>。
</p>
</div>
