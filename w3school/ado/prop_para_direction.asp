...
<div>
<h2>定义和用法</h2>
<p>Direction 属性可设置或返回一个 ParameterDirectionEnum 值，指示 Parameter 所表示的是输入参数、输出参数还是两者都表示，或者指示参数是否是存储过程的返回值。默认值是 adParamInput。</p>

<p>Direction 属性为读/写，它允许用户使用不返回此信息的提供者，或者在不希望 ADO 为检索参数信息而额外调用提供者时设置此信息。</p>

<p>并非所有提供者都能在其存储过程中确定参数的方向。在这种情况下，必须在执行查询前设置 Direction 属性。</p>


<h3>语法</h3>
<pre>objparameter.Direction</pre>
</div>
...
<div>
<h2><a id="parameterdirectionenum">ParameterDirectionEnum Values</a></h2>
