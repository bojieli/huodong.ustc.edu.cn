...
jPushPlugin = {
	...
	setApp: function (obj, screen) {
		...
		var id = $('#application_' + obj.attr('id') + '_list');
	...
	getApplication: function (data) {
		...
		$.ajax({
			...
			url: jPushTemplate.getXmlData('ApplicationData'),
			...
			success: function (xmlHttp) {
				...
				$(xmlHttp).find('xmlData Application app').each(
					...
					function () {
						...
						list = $('<td title="' + obj.attr('name') + '"><div class="applic-theme-img"><img src="' + obj.attr('ico') + '" width="' + wh + '" height="' + wh + '" /></div><div class="applic-theme-tit">' + obj.attr('name') + '</div><div class="applic-theme-explain">' + obj.attr('explain') + '</div></td>').appendTo(tr);
